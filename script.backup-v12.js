...
async function unlockFullEcho() {
  ...
  try {
    const response = await fetch("/api/unlock", {
      ...
      body: JSON.stringify({
        code,
        profileId: lastResult ? lastResult.id : null,
        user: lastResult ? lastResult.user : user,
    ...
    document.getElementById("unlockStatus").textContent = "Backend не е активен. Отвори през http://localhost:3000";
  }
}
...
async function generateUnlockCode() {
  ...
  try {
    const response = await fetch("/api/code", {
      ...
      body: JSON.stringify({ offer, note })
    });

    const result = await response.json();

    if (!result.ok) {
      alert("Не успях да генерирам код.");
      return;
    }

    lastGeneratedCode = result.code.code;

    const out = document.getElementById("generatedCodeOutput");
    if (out) {
      out.textContent = `Нов код: ${lastGeneratedCode}`;
    }

    copyText(lastGeneratedCode);
    await loadBackendData();

    alert("Кодът е генериран и копиран.");
  } catch {
    alert("Backend не е активен. Отвори през http://localhost:3000");
  }
}

function copyGeneratedCode() {
  if (!lastGeneratedCode) {
    alert("Първо генерирай код.");
    return;
  }
...
function renderAdminStats() {
  ...
  if (totalCodes) {
    totalCodes.textContent = adminCodes.length;
  }
}
...
function renderCodesTable() {
  ...
  if (!adminCodes.length) {
    table.innerHTML = `<tr><td colspan="6" class="empty-row">Още няма генерирани кодове.</td></tr>`;
    return;
  }
...
function exportJSON() {
  downloadFile("echoprofile-v11-data.json", JSON.stringify({
    profiles: adminProfiles,
    leads: adminLeads,
    codes: adminCodes
  }, null, 2), "application/json");
...
async function requestPayment(offer, amount) {
  ...
  try {
    ...
    alert("Payment заявката е записана в Admin Pro и текстът е копиран. След ръчно плащане потвърждаваш в админ панела и се генерира код.");
  } catch {
    alert("Backend не е активен. Отвори през http://localhost:3000");
  }
}
...
След потвърждение на плащането ще получите еднократен код за отключване на Full Echo анализа.`;
}
...
async function confirmPayment(paymentId) {
  ...
  try {
    ...
    if (!result.ok) {
      alert(result.error || "Не успях да потвърдя плащането.");
      return;
    }
    ...
    alert(`Плащането е потвърдено. Кодът е генериран и копиран: ${lastGeneratedCode}`);
  } catch {
...
function renderAdminStats() {
  ...
  const totalCodes = document.getElementById("totalCodes");
  if (totalCodes) totalCodes.textContent = adminCodes.length;

  const totalPayments = document.getElementById("totalPayments");
  if (totalPayments) totalPayments.textContent = adminPayments.length;
}
...
function renderPaymentsTable() {
  ...
  adminPayments.slice().reverse().forEach(payment => {
    ...
    tr.innerHTML = `
      ...
      <td><strong>${escapeHTML(codeText)}</strong></td>
      <td>${actionButton}</td>
    `;
...
function exportJSON() {
  downloadFile("echoprofile-v12-data.json", JSON.stringify({
    profiles: adminProfiles,
    leads: adminLeads,
    codes: adminCodes,
...
async function adminFetch(url, options = {}) {
  ...
  if (!pin) {
    throw new Error("Admin PIN липсва.");
  }
...
async function loadBackendData() {
  try {
    ...
    alert("Backend не е активен или Admin PIN липсва. Отвори през http://localhost:3000");
  }
}
...
async function generateUnlockCode() {
  ...
  try {
    const response = await adminFetch("/api/code", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
...
async function confirmPayment(paymentId) {
  ...
  try {
    const response = await adminFetch("/api/payment-confirm", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
...
async function clearAll() {
  const ok = confirm("Да изтрия ли всички backend профили, заявки, кодове и плащания?");
  if (!ok) return;
