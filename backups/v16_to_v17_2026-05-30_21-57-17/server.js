...
const APP_VERSION = process.env.APP_VERSION || "V16_SUPABASE_ADAPTER";
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && (req.url.startsWith("/api/payment-confirm") || req.url.startsWith("/api/admin/payment-confirm"))) {
      ...
      newCode.deliveryText = `Здравей, плащането е потвърдено. Твоят SoulFlame EchoProfile код е: ${newCode.code}. Въведи го в полето за Full Echo, за да отключиш анализа.`;
...
server.listen(PORT, () => {
  console.log("");
  console.log("SoulFlame EchoProfile V16 Supabase Adapter is running.");
