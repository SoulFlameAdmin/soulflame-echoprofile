...
function openCurrentAIReport() {
  if (!lastResult) {
    alert("Първо направи профил.");
    return;
  }

  showAIReport(lastResult);
}

function openAIReport(id) {
  const item = adminProfiles.find(profile => Number(profile.id) === Number(id));

  if (!item) {
    alert("Не намирам профила.");
    return;
  }
...
function showAIReport(profile) {
  const report = generateDeepAIReport(profile);

  currentModalText = report;

  document.getElementById("modalTitle").textContent = "Deep AI Echo Analysis";
  document.getElementById("modalReport").textContent = report;
  document.getElementById("reportModal").classList.remove("hidden");
}

function generateDeepAIReport(profile) {
  ...
  const scoresLocal = {
    ...
    logic: Number(s.logic || 0),
    social: Number(s.social || 0),
    action: Number(s.action || 0)
  };

  const total = Object.values(scoresLocal).reduce((sum, value) => sum + value, 0) || 1;

  const percent = {};
  Object.keys(scoresLocal).forEach(key => {
    percent[key] = Math.round((scoresLocal[key] / total) * 100);
  });

  const sorted = Object.keys(scoresLocal).sort((a, b) => scoresLocal[b] - scoresLocal[a]);
  ...
  const monetization = getMonetizationPath(mainTrait, secondTrait);
  const relationship = getRelationshipPattern(mainTrait, secondTrait);
  const block = getHiddenBlock(mainTrait, weakestTrait);
  const offer = getBestOffer(mainTrait, userData.goal);

  return `DEEP AI ECHO ANALYSIS
Версия: V9 AI-style Analyzer

Име: ${userData.name || "няма"}
Контакт: ${userData.contact || "няма"}
Цел: ${userData.goal || "няма"}
Възраст: ${userData.age || "няма"}
...
ТОЧКИ:
Визия: ${scoresLocal.vision} / ${percent.vision}%
Емоция: ${scoresLocal.emotion} / ${percent.emotion}%
Логика: ${scoresLocal.logic} / ${percent.logic}%
Социалност: ${scoresLocal.social} / ${percent.social}%
Действие: ${scoresLocal.action} / ${percent.action}%

1. ЯДРО НА ЛИЧНОСТТА
Твоят водещ модел е ${main.archetype}. Това означава, че основно функционираш през следната сила: ${main.power}.
Вторият слой — ${second.archetype} — добавя допълнителна посока: ${second.power}.

Ти не си просто един тип. Ти си комбинация. Основният ти слой показва как взимаш посока, а вторият слой показва как я подсилваш или усложняваш.

2. ИНТЕНЗИТЕТ НА ПРОФИЛА
${intensity}

3. ОСНОВНА СИЛА
${main.power}.
Когато си в правилната среда, тази сила може да се превърне в продукт, влияние, връзка, бизнес или лична стратегия.
...
5. ОСНОВЕН РИСК
${main.risk}.
Това не е слабост. Това е мястото, където силата ти може да се обърне срещу теб, ако няма система.

6. БИЗНЕС ПОСОКА
${main.business}

Най-подходяща monetization посока:
${monetization}

7. ВРЪЗКИ И ПАРТНЬОРСТВО
${relationship}

SoulMatch логика:
${main.match}.

8. ПОДХОДЯЩА ОФЕРТА ЗА ТЕБ
${offer}

9. 7-ДНЕВЕН ПЛАН
Ден 1: избери една конкретна посока, не пет.
...
Ден 7: запиши какво работи и го превърни в повторима система.

10. 30-ДНЕВНА СТРАТЕГИЯ
Седмица 1: фокус и диагностика.
Седмица 2: MVP или оферта.
Седмица 3: реални разговори и тестови клиенти.
Седмица 4: продажба, подобрение и повторение.

11. AI TWIN БАЗА
Този профил може да бъде първият слой на AI Twin. 
За AI Twin са нужни още:
- стил на писане
- снимки/визия
- глас
...
12. FINAL ECHO VERDICT
Твоят най-силен ход не е просто да се “разбереш”.
Твоят най-силен ход е да превърнеш профила си в система: как мислиш, как избираш, как продаваш, как обичаш и как действаш.

ALIEN LOGIC:
Човекът не е статична личност. Човекът е динамична програма от вероятности. EchoProfile V9 започва да чете не само какъв си, а накъде можеш да се развиеш, ако правилната система те води.`;
}

function getIntensity(total) {
  if (total >= 44) {
    return "Профилът е с висок интензитет. Това означава, че вътрешните модели са силно активни и човекът вероятно има мощна вътрешна посока, но също така риск от претоварване.";
  }

  if (total >= 32) {
    return "Профилът е със средно-висок интензитет. Има ясни активни черти, но системата още може да се насочи и стабилизира.";
  }

  return "Профилът е с умерен интензитет. Това показва нужда от повече яснота, по-добра среда и по-конкретна посока.";
}

function getHiddenBlock(mainTrait, weakestTrait) {
  const blocks = {
    vision: "Скритият блокаж е прекалено много бъдеще и недостатъчно настояща структура.",
    emotion: "Скритият блокаж е прекалено много усещане и недостатъчно ясна граница.",
    logic: "Скритият блокаж е прекалено много анализ и недостатъчно риск.",
    social: "Скритият блокаж е прекалено много външно влияние и недостатъчно лична посока.",
    action: "Скритият блокаж е прекалено много движение и недостатъчно стратегическо спиране."
  };

  const weakText = {
    vision: "Най-слабият слой е визията, затова трябва да се работи върху по-голяма картина.",
    emotion: "Най-слабият слой е емоцията, затова трябва да се работи върху вътрешно усещане и емпатия.",
    logic: "Най-слабият слой е логиката, затова трябва да се работи върху структура и правила.",
    social: "Най-слабият слой е социалността, затова трябва да се работи върху комуникация и контакти.",
    action: "Най-слабият слой е действието, затова трябва да се работи върху изпълнение и завършване."
  };

  return `${blocks[mainTrait] || "Скритият блокаж е липса на баланс."} ${weakText[weakestTrait] || ""}`;
}

function getMonetizationPath(mainTrait, secondTrait) {
  const combo = `${mainTrait}-${secondTrait}`;

  const map = {
    "vision-logic": "AI/AR продукт, SaaS, EchoProfile, SoulMatch, технологична платформа.",
    "vision-emotion": "психологически AI тестове, личен бранд, съдържание, AI Echo профили.",
    "vision-action": "MVP фабрика, агенция за иновативни продукти, бързи прототипи.",
    "logic-action": "автоматизации, CRM, бизнес системи, scoring engine, dashboard-и.",
    "emotion-social": "връзки, психология, комуникация, съдържание, консултации.",
    "social-action": "B2B продажби, партньорства, локални бизнес оферти, комисионни."
  };

  return map[combo] || "най-добрата посока е малък продукт, който съчетава личната сила с реален проблем на хората.";
}

function getRelationshipPattern(mainTrait, secondTrait) {
  if (mainTrait === "vision") {
    return "Във връзки имаш нужда от човек, който не убива мечтата ти, но те връща към реалност.";
  }

  if (mainTrait === "emotion") {
    return "Във връзки имаш нужда от честност, емоционална безопасност и човек без скрити игри.";
  }

  if (mainTrait === "logic") {
    return "Във връзки имаш нужда от директност, уважение и човек, който не бяга от истината.";
  }

  if (mainTrait === "social") {
    return "Във връзки имаш нужда от човек, който подкрепя контактите ти, но не те кара да се губиш в чужди очаквания.";
  }

  if (mainTrait === "action") {
    return "Във връзки имаш нужда от човек, който издържа на темпото ти, но ти помага да не гориш прекалено бързо.";
  }

  return "Във връзки имаш нужда от баланс между свобода, яснота и сигурност.";
}

function getBestOffer(mainTrait, goal) {
  if ((goal || "").includes("Бизнес")) {
    return "Най-подходяща оферта: Full AI Echo Business Direction — анализ на личността + бизнес роля + 30-дневен план.";
  }

  if ((goal || "").includes("Връзки")) {
    return "Най-подходяща оферта: SoulMatch Echo Analysis — любовен стил + подходящ партньорски тип + червени флагове.";
  }

  if ((goal || "").includes("AI Twin")) {
    return "Най-подходяща оферта: AI Twin Base Blueprint — профил + стил + правила + първа структура за дигитален двойник.";
  }

  return "Най-подходяща оферта: Deep Echo Blueprint — цялостен профил, блокажи, сила, посока и лична стратегия.";
}

function copyModalReport() {
  const text = document.getElementById("modalReport").textContent || currentModalText || "";
  copyText(text);
  alert("AI докладът е копиран.");
}

function downloadModalReport() {
  const text = document.getElementById("modalReport").textContent || currentModalText || "";
  downloadFile("deep-ai-echo-analysis.txt", text, "text/plain");
}
